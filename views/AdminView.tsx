...
const AdminView: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="flex gap-4">
          <div className="bg-white px-6 py-4 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Negocios</p>
          ...
          <div className="bg-white px-6 py-4 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
            <div className="text-right">
              ...
              <p className="text-2xl font-black text-amber-600">{requests.filter(r => r.status === SubscriptionStatus.PENDING).length}</p>
            </div>
      ...
      {/* Tabs */}
      <div className="flex p-1.5 bg-slate-200/50 rounded-2xl w-full max-w-xl mx-auto lg:mx-0">
        <button 
          onClick={() => setActiveTab('requests')}
          className={`flex-1 py-3 px-6 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'requests' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
        >
        ...
        <button 
          ...
          className={`flex-1 py-3 px-6 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'profiles' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
        >
          <Users size={16} /> Negocios
        </button>
        <button 
          onClick={() => setActiveTab('archived')}
          className={`flex-1 py-3 px-6 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'archived' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
        >
        ...
        <div className="space-y-6">
          <div className="bg-white border border-slate-100 rounded-[2.5rem] overflow-hidden shadow-xl">
             ...
             <div className="divide-y divide-slate-100">
               {requests.map(req => (
                 <div key={req.id} className={`p-6 lg:p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6 transition-all ${req.status === SubscriptionStatus.PENDING ? 'bg-amber-50/20' : 'opacity-60'}`}>
                    <div className="flex items-start gap-4">
                      <div className={`p-4 rounded-3xl shrink-0 ${req.status === SubscriptionStatus.PENDING ? 'bg-amber-100 text-amber-600' : req.status === SubscriptionStatus.CONFIRMED ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                         <DollarSign size={28} />
                      </div>
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                           <h4 className="font-black text-slate-800 text-lg uppercase tracking-tight">{req.businessName}</h4>
                           <span className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest ${
                             req.status === SubscriptionStatus.PENDING ? 'bg-amber-100 text-amber-600 border border-amber-200' :
                             req.status === SubscriptionStatus.CONFIRMED ? 'bg-emerald-100 text-emerald-600 border border-emerald-200' :
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-2">
                           <div>
                             ...
                             <p className="text-xs font-bold text-slate-600">{req.months} Mes{req.months > 1 ? 'es' : ''}</p>
                           </div>
                           <div>
                             <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Monto</p>
                             <p className="text-xs font-black text-slate-800">${req.amountUsd}</p>
                           </div>
                           <div>
                             <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Método</p>
                             <p className="text-xs font-black text-blue-600 uppercase">{req.method}</p>
                           </div>
                           <div>
                             <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Referencia</p>
                             <p className="text-xs font-black text-slate-800 bg-slate-100 px-2 py-0.5 rounded-lg inline-block font-mono">{req.reference}</p>
                           </div>
                        </div>
                        <p className="text-[9px] font-bold text-slate-300 mt-3 uppercase tracking-widest">{new Date(req.date).toLocaleString()}</p>
                      </div>
                    </div>
                       ...
                       {req.status === SubscriptionStatus.PENDING && (
                         <>
                           <button 
                             ...
                             className="flex-1 lg:flex-none px-6 py-3 bg-white border border-rose-100 text-rose-600 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-rose-50 transition-all shadow-sm"
                           >
                             Declinar
                           </button>
                           <button 
                             onClick={() => handleProcessRequest(req.id, SubscriptionStatus.CONFIRMED)}
                             className="flex-1 lg:flex-none px-8 py-3 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-emerald-100 hover:shadow-xl transition-all active:scale-95"
                           >
                             Confirmar Pago
                           </button>
                         </>
               ...
               {requests.length === 0 && (
                 <div className="p-20 text-center">
                    ...
                    <p className="text-slate-400 font-black uppercase tracking-widest text-xs">No hay solicitudes de pago aún</p>
                 </div>
               )}
             </div>
          </div>
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
          ...
          {activeTab === 'profiles' && appConfig && (
            <div className="bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-xl">
              <h2 className="text-lg font-black text-slate-800 uppercase tracking-tighter mb-6 flex items-center gap-2">
              ...
              <form onSubmit={handleSaveConfig} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Banco</label>
                  <input 
                    type="text" 
                    value={appConfig.bankName}
                    onChange={(e) => setAppConfig({...appConfig, bankName: e.target.value})}
                    className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-bold text-slate-700" 
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Nro de Cuenta</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Nro de Teléfono</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Cédula de Identidad</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Binance User/ID</label>
                  <input 
                    type="text" 
                ...
                <div className="flex items-end">
                  <button 
                    ...
                    className="w-full bg-slate-900 text-white py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transition-all"
                  >
          ...
          <div className="grid grid-cols-1 gap-6">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-4 space-y-4">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {profile.archived && <span className="bg-slate-100 text-slate-600 text-[10px] font-black px-2 py-0.5 rounded-full uppercase">Archivado</span>}
                        </div>
                      ...
                      <div className="space-y-3 pt-2">
                        ...
                        {profile.lastPaymentRef && (
                          <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3">
                            <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-1">Última Referencia de Pago</p>
                            <p className="text-xs font-black text-emerald-800">{profile.lastPaymentRef}</p>
                          </div>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="text-center space-y-4">
                        ...
                        <div>
                          ...
                          <input 
                            ...
                            className="w-full text-center bg-slate-50 border border-slate-100 rounded-xl py-2 font-black text-slate-800 mt-2 outline-none focus:border-blue-200 transition-all"
                          />
                          <button 
                            onClick={() => addMonths(profile.id, profile.subscriptionExpires, 1)}
                            className="w-full mt-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 py-2 rounded-xl text-[10px] font-black transition-all flex items-center justify-center gap-2 border border-emerald-100"
                          >
                    ...
                    <div className="lg:col-span-4 flex flex-col justify-center space-y-3">
                        <button 
                          ...
                          className={`w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-sm ${profile.isBanned ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-rose-50 text-rose-600 hover:bg-rose-100'}`}
                        >
                        ...
                        <button 
                          ...
                          className={`w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-sm ${profile.archived ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                        >
                        ...
                        <button 
                          ...
                          className="w-full py-2.5 bg-slate-50 border border-slate-100 text-slate-400 hover:text-slate-600 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all"
                        >
                          Limpiar Referencia
                        </button>
                    </div>
            ...
            {filteredProfiles.filter(p => activeTab === 'archived' ? p.archived : !p.archived).length === 0 && (
              <div className="py-32 text-center bg-white rounded-[3rem] border-2 border-dashed border-slate-100">
                {activeTab === 'archived' ? <Archive className="mx-auto text-slate-100 mb-4" size={64} /> : <Users className="mx-auto text-slate-100 mb-4" size={64} />}
